...
class ContactAdapter(private val list: List<ContactItem>, val context: Context) :
...
//                                ContactFragment.changeList()
    ...
    fun openContactsEdit(contactId: String) {
        ...
        val projections =
            arrayOf(ContactsContract.Contacts.LOOKUP_KEY, ContactsContract.Contacts._ID)
        val where = ContactsContract.Contacts._ID + "= ?"
        val values = arrayOf<String>(contactId)
        var mCursor: Cursor? =
            itemView.context.contentResolver.query(phoneUri, projections, where, values, null)
